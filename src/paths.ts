import { useTosi, HookType } from "./use-tosi";
...
export const typedTosi = <S extends object>() => ({
  useTosi: <P extends TosiPath<S> & string>(
    path: P,
    initialValue?: TosiPathValue<S, P>,
  ): HookType<TosiPathValue<S, P>> => useTosi<TosiPathValue<S, P>>(path, initialValue),
});
